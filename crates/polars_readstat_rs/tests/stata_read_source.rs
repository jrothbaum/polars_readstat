//! Exercises `StataReader::open_bytes` against the same fixture corpus used
...
use common::stata_files;
use polars_readstat_rs::StataReader;

#[test]
fn open_bytes_matches_open_path_for_all_stata_files() {
    let files = stata_files();
    assert!(!files.is_empty(), "expected at least one Stata .dta fixture");

    let mut mismatches = Vec::new();

    for path in &files {
        let via_path = match StataReader::open(path) {
            Ok(r) => r,
            Err(e) => {
        ...
        let via_bytes = match StataReader::open_bytes(bytes) {
            Ok(r) => r,
            Err(e) => {
...
fn open_bytes_parallel_matches_sequential() {
    ...
    let bytes = std::fs::read(&path).expect("read fixture bytes");
    let reader = StataReader::open_bytes(bytes).expect("open_bytes");
