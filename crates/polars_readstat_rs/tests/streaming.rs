...
fn stream_and_count(
    ...
    let t0 = Instant::now();
    let mut stream = readstat_batch_iter(
        path,
        Some(opts),
...
fn test_sas_psam_multithread_benchmark() {
    ...
    println!(
        "\npsam_p17.sas7bdat ({} rows, batch_size={})",
...
fn test_sas_pu2023_partial_stream_memory() {
    ...
    println!(
        "\npu2023.sas7bdat ({} total rows × 5200 cols, {} threads)",
        TOTAL_ROWS, THREADS
    );
    println!(
        "{:<28} {:>10} {:>10} {:>10} {:>10}",
        "config", "rows", "batches", "time", "peak RSS"
    );
    println!("{}", "-".repeat(74));
...
fn test_sas_pu2023_stream_discard() {
    ...
    println!(
        "{:<28} {:>10} {:>10} {:>10}",
        "config", "rows", "time", "peak RSS"
    );
    println!("{}", "-".repeat(64));

    for (label, threads) in configs {
        let (rows, batches, elapsed, peak_kb) = stream_with_rss(&path, threads, PU_BATCH, None);
